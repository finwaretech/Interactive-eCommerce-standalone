<!DOCTYPE html>
<!--
Author:Bala Ganeshan V,Raghavendra Acharya
-->
<?php
require('dbconnect.php');
?>

<!-- Documentation of Interactive iCommerce     -Raghav -->
<!--
 The responsiveness of the page is controlled by two variables width and height.
 Other than these variables the 
-->
...
//The value should be taken from the DB
//The below initialization is given as the page gets shrinked if this width and height was not initialized right

$width="100%";
$height="499px";

// This variable status_from_opencart was introduces to link those product links in Opencart to display in Separate from opencart
//This was done when all the open cart links were asked to be redirected to SeparatefromOpencart
$status_from_opencart=isset($_GET['s'])?$_GET['s']:0;
...
// The oc_product_interactivity table in the database has the details of the interactivity 
...
$audio_name_query=mysql_query("SELECT audio_name,audio_condition,audio_time FROM oc_product_interactivity WHERE product_id=".$product_id." ORDER BY interactivity_order ASC;");
...
while($audio_name_value = mysql_fetch_array($audio_name_query)) 
{
	...
	array_push($audio_name, $audio_name_value['audio_name']);//audio name
	array_push($audio_condition, $audio_name_value['audio_condition']);//audio condition
	array_push($audio_time_initial, $audio_name_value['audio_time']);// audio time

}
// var_dump($audio_name);

// The audio names are pushed to another array [Raghav]
...
////////////////////////////////////AUDIO -  [Raghav]   -Stop /////////////////////////////////////////
	...
	// Start Section - Accessories - Separate Products - Same Frame To be checked and deleted
	...
	// End Section - Accessories - Separate Products - Same Frame To be checked and deleted
    ...
    //Accessories values are taken from the table - For future reference
	...
	//Setting number_of_hotspot and heading from database for hotspot [Raghav]
	if($hotspot==1)
	{
	...
	{
		$number_of_hotspot = $row['number_of_hotspot'];//Number of hotspot
		$heading=$row['name'];//Heading of the page
	} 
...
<!-- In the case of hotspot to be responsive we have to get the width and height from hotspot generic page for it to be responsive -->
<?php
$width=isset($_GET['width'])?$_GET['width']:'749px';
$height=isset($_GET['height'])?$_GET['height']:'499px';
$zoom_button=isset($_GET['zoom_button'])?$_GET['zoom_button']:0;// For disabling zoom button
//Here the zoom button is not required for small products like that of taetossian
//To diable this we are adding a zoom_button at the link so that it will disable the zoom button


//Adjusting the width and height for hotspot funtionality
$width_for_iframe=str_replace("px","",$width);
$height_for_iframe=str_replace("px","",$height);
$height_for_2nd_iframe=intval($height_for_iframe);
//Adjusting the width and height for hotspot funtionality
$art=$width_for_iframe;

// making the zoom buttons responsive##############################
...
<!-- Code if responsive there should be a time for each feature Exceptional case -for mockups  Raghav-->

<!-- While creating mockup pages sometimes this should be responsive if in the case we want to set width and height in %  -->
<script type="text/javascript">

var status_for_audio=0,audio_time_value=0;
<?php
...
<!-- Code if responsive there should be a time for each feature  Raghav-->

<!-- Basic CSS for the Product popup - START -->
<style>
/*GROW*/
.grow img 
{
...
.grow1010 img 
		...
		@media only screen and(max-width: 500px){
			.buttons_view_enlarged{
				width:6% !important;
				padding:2px !important;

			}

			.buttons_view_minimised{
...
<!-- Basic CSS for the Product popup - STOP -->
...
<script>
// This script below is for future addition of the patterns 

var product_howler_audio_url="initial";
var product_howler_audio_loop=false;
...
{	
  ...
  $(".interactivity_rotate").click(function()
  {
   // alert("interactivity_rotate !!");
		...
		// The below section is important not Obselete
...
// The audio code is taken into script here
// tHE PHP VALUES ARE TRANSFERED TO JAVASCRIPT USING json
// array_with_audio_position- Has the postion of audio
// array_with_audio_name - Has the name of audio
// array_with_audio_condition - Has the condition of audio loop,single etc
// Special cases will need to run for a particular time [array_with_audio_time]
		...
		$array_with_audio_condition = json_encode($audio_condition);
		echo "var array_with_audio_condition = ". $array_with_audio_condition . ";\n";

		$array_with_audio_time = json_encode($audio_time_initial);
		echo "var array_with_audio_time = ". $array_with_audio_time . ";\n";
		?>
		...
		// alert("song_url: "+"converted/"+product_rotate_running1+"/audio/"+array_with_audio_name[count]);
		product_howler_audio_url = "converted/"+product_rotate_running1+"/audio/"+array_with_audio_name[count];
		$("#product_interactivity_audio").attr("src", "converted/"+product_rotate_running1+"/audio/"+array_with_audio_name[count]);
		if(array_with_audio_condition[count] === 'loop')
		{
			$("#product_interactivity_audio").attr("loop",true);
			product_howler_audio_loop = true;
		}
		else
			$("#product_interactivity_audio").attr("loop",false);
			product_howler_audio_loop = false;
		}
		if(array_with_audio_time[count] != null)
		{
			status_for_audio=1;
			audio_time_value=parseInt(array_with_audio_time[count]);
			// alert("initial status"+status_for_audio);
		}
...
// ///////////////////////////The above function is the one which calls real.js//////////////////////////////////  // 
   ...
   $(".interactivity_rotate100").click(function(){
        ...
        // alert("song_url: "+"image/upload/<?php echo $product_id; ?>/audio/"+audio_string_split[Math.floor(Math.random()*audio_string_split.length)]);
...
<script type="text/javascript">
// Important function Written by Bala      -Raghav
function R360(interactivity,productid,a,b,c,d,e,f,g,h)
{
 //alert(h);
	...
	if(h == '360')
	{
		...
		rows=0;
		cross = false; // For 360*360 interactivity. 
	}
...
// The timing and zoom button code is done here
// In reel.js there is a variable called balastop which stops the rotation and removes the zoom button // 
var stop_time;
<?php
	if($zoom_button==1)
	{
	?>
		stop_time=1000;// To disable zoom button
	<?php
	}
	?>
	// The hotspot case
	...
	?>
// If there is a audio time that time has to be initialized
if(status_for_audio===1)
{
	stop_time=audio_time_value;
}

<?php
// if($hotspot==1)

if(($hotspot==1)||($zoom_button==1))
{
		?>
		$(function(){ 
			$('#product_rotate_image').reel({
				...
				zoomheight: 2000,
				balastop: stop_time,
				product_howler_audio_url_reel: product_howler_audio_url,
				product_howler_audio_loop_reel: product_howler_audio_loop
				/*zoomwidth: 1152, 
				zoomheight: 768*/
				/*zoomwidth: 768, 
				zoomheight: 512,*/		
...
if(($hotspot!=1)&&($zoom_button!=1))
		{
		?>
		// alert("product_howler_audio_url"+product_howler_audio_url);
		// alert("product_howler_audio_loop"+product_howler_audio_loop);
		$(function(){ 
			$('#product_rotate_image').reel({
...
<!-- Generalisation for opencart width solution -->

<?php
if($status_from_opencart==1)
{
	// $x=$name[1];
?>
<script type="text/javascript">
...
<!-- If there is a hotspot then there should be a back button also  else just the heading-->

<?php
if($hotspot!=1)
{
?>
	<div class="heading-reveal-modal heading_reveal">
		<?php echo $heading; ?>
	</div>
<?php
}
...
{
?>
  <div class="heading-reveal-modal heading_reveal" width="<?php echo $width_for_iframe;?>"><?php echo $heading;?></div>
    <div id="back_button" style="visibility:hidden;" type="button" value="Home" class="button back_button"><img style="cursor:pointer;" src="https://www.3dwebcommerce.com/Separatefromopencart/back.png"></div>


<iframe src="Hotspot/hotspot_generic.php?id=<?php echo $product_id;?>" name="frame2" id="frame2" frameborder="0" scrolling="no" overflow="hidden" overflow-y="hidden" seamless="seamless" marginwidth="0" marginheight="0" onload="" allowtransparency="false" style="width:<?php echo $width;?>;height:<?php echo $height;?>; margin-left:5px;margin-top:7%;"></iframe><div class="action"><input type="button" value="Back" class="closePopup1"/ style="display: none; margin-top: 591px;color: white;background-color: rgb(11, 174, 249);height: 24px;border-radius: 7px;font-size: 11px;font-family: Arial, Helvetica, sans-serif;padding: 0px 12px 0px 12px;"></div>
 <?php
}
?>
<?php
...
<iframe src="Hotspot/hotspot_generic.php?status_for_audio=1&id=<?php echo $product_id;?>" name="frame2" id="frame2" frameborder="0" scrolling="no" overflow="hidden" overflow-y="hidden" seamless="seamless" marginwidth="0" marginheight="0" onload="" allowtransparency="false" style="width:<?php echo $width;?>;height:<?php echo $height;?>; margin-left:5px;"></iframe><div class="action"><input type="button" value="Back" class="closePopup1"/ style="display: none; margin-top: 591px;color: white;background-color: rgb(11, 174, 249);height: 24px;border-radius: 7px;font-size: 11px;font-family: Arial, Helvetica, sans-serif;padding: 0px 12px 0px 12px;"></div>
 <?php
}
?>
...
<!-- Appending the width and height along with the hotspot_generic link above -->
<script type="text/javascript">
	 // var src = $('#frame2').attr('src');
	 // Appending the width and height along with the hotspot_generic link above
	 var srce = document.getElementById("frame2").src;
	 var width_for_iframe=parseInt(<?php echo $width_for_iframe;?>);
	 var height_for_iframe=parseInt(<?php echo $height_for_iframe;?>);
	 // alert(width_for_iframe+" : "+height_for_iframe);
	 // alert(src);
	 srce=srce+"&width="+width_for_iframe+"px&height="+height_for_iframe+"px";
	 // alert(srce);
    document.getElementById("frame2").src=srce;
...
<!-- Start Section - Video Frame -->
 ...
 <!-- Start Section - Accessories - Same Frame - Same product - -->
 ...
 // End Section - Accessories - Same Frame - Same product - 
...
<!-- To display the buttons or not [Hotspot dosnot need the buttons]-->
<?php 
if($hotspot!=1)
{
?>
...
<?php
$width_for_responsive_button=intval($art);
if($width_for_responsive_button<500)
{
	?>

	<script type="text/javascript">
	alert($('#buttons_view_enlarged1').width());
	// var theImg = document.getElementById('buttons_view_enlarged1');
	// alert(theImg.width);
	// theImg.width = 25;
	// alert(theImg.width);
			...
			// $('#buttons_view_enlarged1').width('25px');
	</script>
